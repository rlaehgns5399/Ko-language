...
public class KoLangVisitor extends HelloBaseVisitor<String>{
	public Procedure procedure = new ProcedureImpl();
	StringBuilder sb = new StringBuilder();
	
	Stack<String> localVariableStack = new Stack<String>();
	@Override
	...
	public String visitImport_decl(HelloParser.Import_declContext ctx) {
		...
		for(int i = 1; i < ctx.getChildCount(); i++){
			import_name += ctx.getChild(i).getText();
		}
		procedure.storeCodesImported(import_name);
		sb.append("import ");
	...
	public String visitInterface_decl(HelloParser.Interface_declContext ctx) {
		...
		procedure.initInterfaceInfo(ctx.ident().getText());
		super.visitInterface_decl(ctx);
		procedure.setCurrentInterfaceName(null); 	// make null
		return "";
	}
	...
	public String visitInterface_method(HelloParser.Interface_methodContext ctx) {
		 ...
		 * 
		 */
		procedure.saveAbstractMethodInfo(abstractMethodName, numOfParams);
		sb.append("public Object ");
		visit(ctx.getChild(0));	// IDENT
		sb.append("(");
		if(ctx.params() != null){
			visit(ctx.getChild(2));	// visit param
		}
		sb.append(");");
		return "";
	}
	...
	public String visitClass_decl(HelloParser.Class_declContext ctx) {
		 ...
		 * @return void
		 */
		procedure.initInterfaceInfo("temp");
		return "";
	}
...
//		printClassvariables();
	...
	public String visitReturn_stmt(HelloParser.Return_stmtContext ctx) {
		...
		if(ctx.getChildCount() != 1) {
			boolean returnTargetIsValid = procedure.returnTargetIsValid(ctx, localVariableStack);
